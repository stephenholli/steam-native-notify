...
- **Windows support is EXPERIMENTAL**, validated on real Windows 11 but not
  ...
  No vendored binaries; every registration is per-user and reversible. A
  click brings the Steam window to the foreground, from minimized too
  (measured on a native Windows 11 install; a VM pass had not, and
  `docs/platforms.md` records both). The in-game half is further limited by
  Focus Assist, which suppresses toasts during fullscreen games by default.
