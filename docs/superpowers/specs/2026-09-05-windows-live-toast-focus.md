...
Bring Steam to the foreground when a user clicks a live Windows notification
banner, while preserving the existing handler-replay click route.
...
- WScript.Shell.AppActivate with the visible steamwebhelper process ID
  foregrounds Steam from a live banner callback
...
Activated, find the visible top-level steamwebhelper window titled Steam and
call the built-in WScript.Shell.AppActivate method with its process ID.
...
today. Focus handling must never block toast delivery or handler replay.

## Scope

- fix live-banner foregrounding on Windows
...
- fire an achievement notification and verify a live-banner click both
  invokes the route and foregrounds Steam
