<!-- GENERATED by tools/gen-types-table.mjs. Do not edit; run `bun run gen:table`. -->
...
- **35** types have a durable fallback, including media and playtime action tokens
...
| 9 | `GroupChatMessage` | that chat room’s dialog | none; exact replay only, because the room dispatcher requires session-only toast context | client, bundle |
...
| 14 | `Screenshot` | that screenshot in the Media dialog | `action:screenshot:<screenshot_handle>`; `action:media` without a handle | client, bundle |
...
| 45 | `PlaytimeWarning` | playtime dialog | `action:requestplaytime` | client, bundle |
...
| 56 | `GameRecordingStop` | that clip in the Media dialog | `action:clip:<clip_id>`; `action:media` without a clip id | client, bundle |
| 57 | `GameRecordingUserMarkerAdded` | explicit no-op | none | client, bundle |
| 58 | `GameRecordingInstantClip` | that clip in the Media dialog | `action:clip:<clip_id>`; `action:media` without a clip id | client, bundle |
